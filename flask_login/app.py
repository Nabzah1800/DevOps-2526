...
def register():
    ...
    # Basic validatie
    if len(username) < 3:
        return render_template("register.html", error="Username moet minstens 3 tekens zijn.")
    if len(password) < 6:
        return render_template("register.html", error="Password moet minstens 6 tekens zijn.")

    password_hash = generate_password_hash(password)  # salted hash (pbkdf2)
    ...
    try:
        with db_conn() as conn:
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO users (username, password_hash, firstname, lastname, created_at) VALUES (?, ?, ?, ?, ?)",
                (username, password_hash, firstname, lastname, created_at),
            )
            conn.commit()
